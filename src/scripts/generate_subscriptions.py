...
def gen_encontro():
    ...
    data = {
        ...
        'caracteristicas_1': random.choice([(v,v) for v in ['ficção', 'documentário', 'mockumentary', 'docudrama', 'outro(s)']]),
        'caracteristicas_2': random.choice([(v,v) for v in ['sim', 'não']]),
        'caracteristicas_3': random.choice([(v,v) for v in ['curta', 'média', 'longa']]),
        'veiculo': random.choice([(v,v) for v in ['Celular', 'Cinema', 'Streaming', 'TV', 'Outros']]),
        'genero': random.choice([(v,v) for v in ['Comédia', 'Comédia dramática', 'Drama', 'Dramédia', 'Épico', 'Farsa', 'Histórico', 'Híbrido', 'Musical', 'Tragédia', 'Outro']]),
