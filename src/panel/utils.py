...
INDICACOES_AVALIACAO = {
    ...
    LABORATORIO: {ROLE_CURADOR: {1: [(6, 'indica_projeto', 'Melhor projeto')], 
                                 2: [(2, 'indica_projeto', 'Melhor projeto')]} },
    MOSTRA: {ROLE_CURADOR: {1: [(2, 'indica_curta', 'Selecionado para a Mostra'), (1, 'premio_sina', 'Melhor filme com temática social')]}, 
            ...
            for ind in indicacoes_avaliacao(sess['contest'], sess['role'][0], sess['step']):                
                if questions[ind[1]] == 'sim':
                    ...
                    if qtd_ind[ind[1]] >= ind[0]:
                        msg_indicados_lst.append('"{}":"{}: {}"'.format(ind[1], ind[2], titulo_indicado.replace('\'',' ').replace('\"','')))
        
        item.msg_indicados = '{' + ','.join(msg_indicados_lst) + '}'
        try:
